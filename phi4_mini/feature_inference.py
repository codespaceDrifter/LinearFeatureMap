...
class Phi4FeatureInference:
    ...
    def _load_saes(self, sae_path):
        ...
        # load mlp SAEs (layers 0-30) in fp16 to save VRAM
        ...
        # load att SAEs (layers 1-31) in fp16 to save VRAM
        ...
        def hook(module, input, output):
            ...
            with torch.no_grad():
                if kind == "mlp" and layer_idx in self.mlp_saes:
                    feat = self.mlp_saes[layer_idx].encode(output.half())  # (batch, seq, hidden_dim)
                    self.features[f"layer_{layer_idx}_mlp"] = feat.detach()
                elif kind == "att" and layer_idx in self.att_saes:
                    feat = self.att_saes[layer_idx].encode(output.half())  # (batch, seq, hidden_dim)
